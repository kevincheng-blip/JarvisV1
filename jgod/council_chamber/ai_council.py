...
#
# === 儲存幕僚會議室 log 的輔助函式（依 dev/prod 寫入詳略） ===
def save_war_room_log(question, raw_output, final_summary):
    """
    將幕僚會議室結果存成 logs/war_room/YYYY-MM-DD_HH-MM-SS.json，依 dev/prod 寫入詳略，回傳檔案路徑。
    """
...
def save_war_room_log(question, raw_output, final_summary):
    """
    將每次幕僚會議室結果存成 logs/war_room/日期時間.json，不覆蓋舊檔，並回傳路徑。
    """
...
def save_war_room_log(question, raw_output, final_summary):
    """
    將每次幕僚會議室結果存成 logs/war_room/日期時間.json，不覆蓋舊檔。
    """
    log_dir = Path(__file__).resolve().parents[2] / "logs" / "war_room"
    log_dir.mkdir(parents=True, exist_ok=True)
...
def summarize_council_output(raw_output: str) -> str:
    """
    用 OpenAI GPT（或你指定的 provider）將幕僚會議室所有角色輸出整合成一段「最終結論」。
    """
...
# 載入角色設定
with open("config/council_chamber_roles.yaml", "r", encoding="utf-8") as f:
...
def ask_role(
    ...
    """
    v3：單輪幕僚會議室 + 優先使用 YAML 的 system_prompt。
...
def run_war_room(
    ...
    if is_dev_mode():
        print(f"[J-GOD] 幕僚會議室 log 已儲存：{log_path}")
