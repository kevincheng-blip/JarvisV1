"""
AI Council Chamber Engine v4.0 - 核心聊天引擎
乾淨、可維護、可擴充的多 AI 幕僚會議室引擎
"""
import asyncio
import time
import logging
...
from jgod.council_chamber.core.models import (
    RoleName,
    ProviderKey,
    RoleResult,
    WarRoomResult,
...
from jgod.council_chamber.providers.base_provider import BaseProviderAsync, ProviderResult
...
class WarRoomEngine:
    """AI Council Chamber Engine v4.0 - 多 AI 幕僚會議室核心引擎"""
    
    def __init__(self, provider_manager: ProviderManager):
        """
        初始化 AI Council Chamber Engine
    ...
    async def run_war_room(
        ...
        Returns:
            WarRoomResult
        """
        self.logger.info(f"=== AI Council Chamber Engine v4.0 Execution ===")
        ...
        try:
            ...
            # 記錄最終結果
            self.logger.info(f"AI Council Chamber execution completed:")
            ...
            self.logger.exception("AI Council Chamber execution failed with exception")
            # 即使失敗，也返回已完成的結果
            role_results: Dict[RoleName, RoleResult] = {}
            for task in tasks:
                if task.done():
