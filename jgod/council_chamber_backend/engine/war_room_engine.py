"""
AI Council Chamber Engine v5.0 - 後端版本
...
class WarRoomEngineBackend:
    """AI Council Chamber Engine 後端版本（支援 WebSocket streaming）"""
    ...
    async def run_war_room(
        ...
        """
        執行 AI Council Chamber 分析（async generator，支援真正的 streaming）
        
        Yields:
            WarRoomEvent: 各種幕僚會議室事件（包含即時 chunk）
        """
        try:
            # 取得啟用的 Provider
            enabled_providers = self._get_enabled_providers(mode, custom_providers)
            self.logger.info(f"AI Council Chamber session {session_id} started: mode={mode}, providers={enabled_providers}")
            ...
            for role in RoleName:
                ...
                system_prompt = f"你是 J-GOD 幕僚會議室的{role.value}，負責提供專業分析。"
            ...
            self.logger.info(f"AI Council Chamber session {session_id} completed: {completed_count} roles, {event_count} events")
            ...
            self.logger.error(f"AI Council Chamber execution failed: {e}", exc_info=True)
            yield ErrorEvent(
                session_id=session_id,
