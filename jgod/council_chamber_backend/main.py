"""
AI Council Chamber Backend v5.0 - FastAPI 主程式
"""
import logging
import os
...
if not WAR_ROOM_API_KEY:
    logger.warning(
        ...
        "AI Council Chamber API v5.0 is running WITHOUT authentication protection. "
        "This is only suitable for local development. "
        "DO NOT deploy to production without setting the API key."
    )
else:
    logger.info("✓ AI Council Chamber API v5.0 authentication protection is enabled")

# 建立 FastAPI 應用
app = FastAPI(
    title="J-GOD AI Council Chamber Backend v5.0",
    description="真正即時串流的幕僚會議室後端",
    version="5.0.0",
)
...
async def startup():
    """啟動事件"""
    logger.info("AI Council Chamber Backend v5.0 starting up...")


@app.on_event("shutdown")
async def shutdown():
    """關閉事件"""
    logger.info("AI Council Chamber Backend v5.0 shutting down...")
