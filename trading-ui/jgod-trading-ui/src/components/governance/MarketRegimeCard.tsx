...
type Props = {
  ...
  marketComplexity?: string | number;
  isLoading?: boolean;
  error?: unknown;
};

export function MarketRegimeCard({ data, marketComplexity, isLoading, error }: Props) {
  if (isLoading) {
    return (
  ...
  const regime = data || {};
  const complexity = safeString(marketComplexity);
  const reasons = Array.isArray(regime.reasons) ? regime.reasons.slice(0, 3) : [];

  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
      <div className="flex justify-between items-start mb-2">
        <div className="text-base font-semibold text-gray-900 dark:text-white">Market Regime</div>
        {regime.is_stub && (
      ...
      <div className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
        {safeString(regime.status)}
      </div>
      <div className="text-xs text-gray-600 dark:text-gray-400 mb-2">
        Market Complexity: {complexity}
      </div>
      <div className="text-xs text-gray-600 dark:text-gray-400 mb-3">
        Updated: {safeString(regime.updated_at)}
      </div>
