...
# -*- coding: utf-8 -*-
"""
處理 股市大自然萬物修復法則.txt，建立 AI 知識庫版和閱讀強化版
"""
...
def classify_content(line):
    ...
    # 檢測程式碼
    if '```' in line or 'Python' in line or 'class ' in line or 'def ' in line or 'import ' in line or 'from ' in line or '->' in line or (': ' in line and ('str' in line or 'int' in line or 'float' in line or 'bool' in line or 'Dict' in line or 'List' in line or 'Optional' in line)):
        return '[CODE]'
    
    # 檢測公式
    if '=' in line and ('E_' in line or 'F_' in line or 'O_' in line or '\\' in line or 'λ' in line or 'β' in line or 'α' in line or 'Z-score' in line or 'Zscore' in line):
        return '[FORMULA]'
    
    # 檢測表格
    if '\t' in line and ('模態' in line or '階段' in line or '說明' in line or '用途' in line or '|' in line or '名稱' in line or '目標' in line):
        return '[TABLE]'
    
    # 檢測規則
    if any(keyword in line for keyword in ['如果', '若', '當', '則', '應該', '必須', '需要', '規則', '條件', 'IF', 'THEN', '→', '觸發', '一旦']):
        return '[RULE]'
    
    # 檢測系統架構
    if any(keyword in line for keyword in ['系統', '架構', '模組', '引擎', '流程', '階段', 'Blueprint', 'Engine', '策略', '方法', '模態', 'Cycle', 'Principle']):
        return '[STRUCTURE]'
    
    # 檢測概念定義
    if any(keyword in line for keyword in ['定義', '概念', '特徵', '原則', '邏輯', '哲學', '價值觀', '思維', '理論', '核心', '目的', '目標', '熵', '反熵', 'Alpha', 'Beta']):
        return '[CONCEPT]'
    
    # 預設為註解
    return '[NOTE]'

def process_nature_ai_kb():
    """建立 AI 知識庫版"""
    input_file = '/Users/kevincheng/JarvisV1/docs/股市大自然萬物修復法則.txt'
    output_file = '/Users/kevincheng/JarvisV1/docs/股市大自然萬物修復法則_AI知識庫版_v1.md'
...
def process_nature_reading():
    """建立閱讀強化版"""
    output_file = '/Users/kevincheng/JarvisV1/docs/股市大自然萬物修復法則_閱讀強化版_v1.md'
    
    output = """# 股市大自然萬物修復法則：閱讀強化版 v1
...
if __name__ == '__main__':
    print("開始處理 股市大自然萬物修復法則.txt...")
    process_nature_ai_kb()
    process_nature_reading()
    print("✓ 處理完成！")
